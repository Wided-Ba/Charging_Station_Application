...
{
    ui->setupUi(this);
    ui->tableView_station->setObjectName("tableau1");
    ui->tableView_station_2->setObjectName("tableau2");
    //affichage tableau1
  //***********arduino******************************
    ...
    //**********proposition**************************
       ...
       connect(timer3, &QTimer::timeout, this, &station::arduino);
         timer3->start(5000); // Appeler toutes les secondes (3000 ms)*/
...
void station::appelfonc()
{
    qDebug() << "Appel de la fonction appelfonc()";
    ...
    qDebug() << "Fin de la fonction appelfonc()";
}
...
QPixmap * station::statistique()
{
    ...
    QPieSeries *series = new QPieSeries();
     ...
     if(number4!=0)
     {
             ...
             chart->setTitle("Pourcentage de stations par type");
             chart->legend()->hide();
...
void station::afficherImageSelonMode() {
    ...
    if (query.exec() && query.next()) {
        ...
        if (count > 0) {
            QPixmap image(":/ICON/economie-denergie.png"); // charger l'image X depuis les ressources de l'application
            ui->label_4->setPixmap(image);
        } else {
            QPixmap image(":/ICON/economie-denergie2.png"); // charger l'image Y depuis les ressources de l'application
            ui->label_4->setPixmap(image);
        }
    } else {
...
//*****************Notification*************************
void station::checkMaintenanceDate()
{
  QString message = ""; // Initialiser la chaîne de messages
    // Récupération des informations de toutes les stations
    QSqlQuery query;
    query.prepare("SELECT ID_ST, DDM_ST FROM STATIONS");
    if (!query.exec()) {
        qDebug() << "Erreur lors de l'exécution de la requête : " << query.lastError().text();
        return;
    }
    // Creation de notufication
         QSystemTrayIcon* trayIcon = new QSystemTrayIcon();
         QIcon icon = QIcon(":/ICON/notification.png");
         trayIcon->setIcon(icon);
    // Parcours des stations et vérification des dates de maintenance
    while (query.next()) {
        QString nomStation = query.value(0).toString();
        QDate dateMaintenance = query.value(1).toDate();
        QDate dateLimite = QDate::currentDate().addDays(3);
        if (dateMaintenance <= dateLimite) {
            qDebug() << "La station " << nomStation << " doit être maintenue avant le " << dateMaintenance.toString("dd/MM/yyyy");

                       // Définir le message de notification

                       message += "\nLa station " + nomStation + " doit être maintenue avant le " + dateMaintenance.toString("dd/MM/yyyy") + "\n";
                   }
    }
           ui->textBrowser_2->setText(message);
}

void station::on_pushButton_info_clicked()
{
    QMessageBox::information(this, "Maintenance requise", "Il est important de maintenir les stations dans la date pour assurer leur bon fonctionnement.\nVeuillez maintenir la station pour désactiver la notification.\n\nPour ce faire, veuillez consulter la fiche technique de la station pour connaître la date de maintenance prochaine et le modifier.");
}
